...
class Rawread(Test):
    ...
    def setUp(self):
        ...
        tarball_path = os.path.join(os.path.abspath(''),
                                    "rawread.py.data/rawread.tar")
        tarball = self.get_data('rawread.tar')
        archive.extract(tarball_path, self.teststmpdir)
    ...
    def test(self):
        ...
        if err_val:
            self.fail("test failed for values : %s" % err_val)
