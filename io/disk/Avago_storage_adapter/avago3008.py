...
class Avago3008(Test):
    ...
    def setUp(self):
        ...
        if self.raidlevel == 'raid0':
            self.raid_disk = " ".join(self.disk).strip(" ")
        self.current_operation = "Background"
    ...
    def sleepfunction(self, v):

        """
        This function waits, till the current operation is complete
        """
        while self.current_operation != 'None':
            time.sleep(10)
            self.adapter_status()
            cmd = "./sas3ircu 0 status | grep 'Current operation' | \
                 awk '{print $4}'"
            self.current_operation = process.system_output(cmd, shell=True) \
                .strip("\n")
    ...
    def consistcheck(self):
        ...
        self.current_operation = "Consistancy check"
        self.sleepfunction()
    ...
    def rebuild(self):
        ...
        while self.status != 'Optimal':
            time.sleep(30)
            self.status = self.adapter_status()
            cmd = "./sas3ircu 0 status | grep 'Volume state' | \
                 awk '{print $4}'"
            self.status = process.system_output(cmd, shell=True).strip("\n")
