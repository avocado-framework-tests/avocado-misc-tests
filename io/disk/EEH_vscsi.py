...
class EEHVscsi(Test):
    ...
    def setUp(self):
        ...
        self.eeh_tool = self.params.get('eeh_tool', default=None)
        self.errinjct_cmd = self.params.get('errinjct_cmd', default=None)

        if not self.disk_input:
            self.cancel("disk parameter is required in the YAML configuration")
        if not self.vios_ip:
        ...
        if not self.eeh_tool and not self.errinjct_cmd:
            self.cancel(
                "Either eeh_tool or errinjct_cmd parameter is required "
    ...
    def test_eeh_vscsi(self):
        ...
        if not self._check_reset_started():
            self.fail(
                "LPAR driver did not detect transport disruption within "
                f"{INJECT_CONFIRM_SECS}s — EEH injection may not have "
                "reached the client"
            )
