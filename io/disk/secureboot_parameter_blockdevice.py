...
class SecureBootBlockDevice(Test):
    ...
    def setUp(self):
        ...
        self.driver = self.params.get('block_driver', default='').strip()
    ...
    def _discover_nvme_pcie(self):
        ...
        if candidates:
            ...
            if devices:
                return devices
            # No candidates have namespaces — fall through to nvme list path.
            self.log.info(
    ...
    def test_device_visibility(self):
        ...
        if nvme_devs:
            ...
            for dev in nvme_devs:
                ...
                nqn = next(
                    (d['nqn'] for d in self.nvmf_details
                     if d['dev'] == dev), None
                )
                ...
                if not ns_visible:
                    failures.append(
                        ...
                        f"with controllers {siblings} after Secure Boot enable"
                    )
        ...
        if self.driver in _NVME_DRIVERS:
            ...
            for ctrl in nvme_devs:
                ctrl_base = os.path.basename(ctrl)
                nqn = next(
                    (d['nqn'] for d in self.nvmf_details
                     if d['dev'] == ctrl), None
                )
    ...
    def test_lockdown_state(self):
        ...
        self.log.info("Checking %s", lockdown_path)
        lockdown_line = genio.read_one_line(lockdown_path)
        self.log.info(lockdown_line)
        ...
        # Cancel (not fail) when Secure Boot is not enabled: lockdown=[none]
        # and DT ibm,secure-boot != 2 means the precondition is not met.
        if active_mode == 'none' and not sb_enabled:
            self.cancel(
                "Secure Boot is not enabled on this LPAR "
                "(lockdown=[none], ibm,secure-boot != 2); "
                "enable Secure Boot via HMC and reboot before running"
            )
    ...
    def test_lockdown_enforcement(self):
        ...
        '''
        lockdown_path = '/sys/kernel/security/lockdown'
        if genio.is_pattern_in_file(lockdown_path, r'\w'):
            lockdown_line = genio.read_one_line(lockdown_path)
            match = re.search(r'\[(\w+)\]', lockdown_line)
    ...
    def _check_lpfc(self, failures):
        ...
        if self.nvmf_details:
            self.log.info("[lpfc] Delegating NVMe-oF FC checks to _check_nvmf")
