...
class NVMeTCPSoftTarget(Test):
    ...
    def setUp(self):
        ...
        p_iface = self.params.get(
            'network_config_primary_interface', default=None
        )
        if not p_iface:
            self.cancel("network_config_primary_interface is required")

        p_ip = self.params.get('network_config_primary_ip')
        p_mask = self.params.get('network_config_primary_netmask')
        p_gw = self.params.get('network_config_primary_gateway', default=None)
        p_mtu = self.params.get('network_config_primary_mtu', default=1500)
        ...
        s_iface = self.params.get(
            'network_config_secondary_interface', default=None
        )
        if s_iface:
            s_ip = self.params.get('network_config_secondary_ip')
            s_mask = self.params.get('network_config_secondary_netmask')
            s_gw = self.params.get(
                'network_config_secondary_gateway', default=None
            )
            s_mtu = self.params.get(
                'network_config_secondary_mtu', default=1500
            )
        ...
        if self.ns_mode == 'create':
            ...
            self.ns_config['ns_count'] = self.params.get(
        ...
        # Validate controller exists early so the test cancels before any
        # configuration work when the device is absent
        if self.ns_mode == 'create':
            controller = self.ns_config.get('nvme_controller')
            if not controller:
                self.cancel("namespace_config_nvme_controller is required "
                            "for create mode")
            status, _, _ = self.run_remote_cmd(
                f"test -e {controller}", ignore_status=True
            )
            if status != 0:
                self.cancel(
                    f"Controller {controller} not found on remote host"
                )
    ...
    def run_remote_cmd(self, cmd, ignore_status=False):
        ...
        try:
            ...
            if exit_status != 0 and not ignore_status:
                # Log all diagnostics before raising — fail() raises an
                # exception so nothing after it executes
                self.log.error(
    ...
    def _install_required_packages(self):
        """
        Detect distro, build package list, and install any missing packages
        """
        self.log.info("Checking and installing required packages...")

        _, distro_raw, _ = self.run_remote_cmd(
            "cat /etc/os-release | grep ^ID=", ignore_status=True
        )
        distro = distro_raw.lower()

        pkg_list = ['nvme-cli', 'nvmetcli']
        if any(d in distro for d in ['rhel', 'fedora', 'centos']):
            handler = 'dnf'
            pkg_list.append('NetworkManager')
        elif any(d in distro for d in ['suse', 'sles', 'opensuse']):
            handler = 'zypper'
            pkg_list.append('NetworkManager')
        elif 'ubuntu' in distro or 'debian' in distro:
            handler = 'apt-get'
            pkg_list.append('network-manager')
            self.run_remote_cmd("apt-get update", ignore_status=True)
        else:
            self.cancel(f"Unsupported distro for package management: {distro}")

        for pkg in pkg_list:
            status, _, _ = self.run_remote_cmd(
                f"rpm -q {pkg}" if handler != 'apt-get'
                else f"dpkg -l {pkg} | grep '^ii'",
                ignore_status=True
            )
            if status != 0:
                self.log.info(f"Installing missing package: {pkg}")
                self.run_remote_cmd(f"{handler} install -y {pkg}")
            else:
                self.log.info(f"Package {pkg} already installed")

        self.log.info("All required packages verified")
    ...
    def _get_ns_size(self, unallocated_capacity, block_size, ns_count):
        """
    ...
    def _find_namespace_device(self, controller, ns_id, existing_devices):
        ...
        status, stdout, _ = self.run_remote_cmd(
            "nvme list", ignore_status=True
        )
        if status == 0:
            for line in stdout.strip().split('\n'):
                parts = line.split()
                if (parts
                        and parts[0].startswith(f'/dev/{controller_name}n')
                        and parts[0] not in existing_devices):
                    ns_device = parts[0]
                    self.log.info(
        ...
        if not ns_device:
            status, stdout, _ = self.run_remote_cmd(
                f"ls -1 /dev/{controller_name}n*", ignore_status=True
            )
            if status == 0:
    ...
    def _create_and_attach_namespace(self, controller, ns_size, cont_id,
                                     existing_ns_ids, namespaces):
        """
        Create, attach, and verify a single namespace

        :param controller: NVMe controller device path
        :param ns_size: Namespace size in blocks
        :param cont_id: Controller ID string
        :param existing_ns_ids: Set of already-existing namespace IDs (mutated)
        :param namespaces: List of already-found device paths (to detect new)
        :return: Device path of the newly created namespace
        """
    ...
    def _create_namespaces(self):
        ...
        """
        controller = self.ns_config.get('nvme_controller')
        ns_count = self.ns_config.get('ns_count', 1)

        self.log.info(
            f"Creating {ns_count} namespaces on {controller.split('/')[-1]}..."
        )

        capacity_info = self._get_controller_capacity_info(controller)
        ns_size = self._get_ns_size(
        ...
        for i in range(ns_count):
            ...
            ns_device = self._create_and_attach_namespace(
                controller, ns_size, cont_id, existing_ns_ids, namespaces
            )
