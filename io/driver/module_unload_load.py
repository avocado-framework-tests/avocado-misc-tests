...
class ModuleLoadUnload(Test):
    ...
    def module_load_unload(self, module):
        ...
        """
        if self.built_in_module(module) is True and self.only_io is True:
            self.fail("%s is built_in Module can't be unloaded" % module)
        else:
            self.log.info("%s is built_in Module can't unload" % module)
