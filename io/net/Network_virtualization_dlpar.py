...
class NetworkVirtualization(Test):
    '''
    Adding and deleting Network Virtualized ports from the vios
    '''
    ...
    @skipIf(IS_POWER_NV or IS_KVM_GUEST, "This test is not supported on KVM \
            guest or PowerNV platform")
    ...
    def test(self):
        ...
        for val in range(self.count):
            self.network_virtualization__add()
    ...
    @skipIf(IS_POWER_NV or IS_KVM_GUEST, "This test is not supported on KVM \
            guest or PowerNV platform")
    def network_virtualization__add(self):
        '''
        ...
        if 'slot_num=%s' % self.slot_num not in str(output):
            self.log.debug(output)
            self.fail("lshwres fails to list Network virtualized interface \
                       after add operation")
