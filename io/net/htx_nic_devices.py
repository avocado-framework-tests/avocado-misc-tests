...
class HtxNicTest(Test):
    ...
    def is_net_devices_in_peer_mdt(self):
        ...
        #output = " ".join(output.stdout.decode("utf-8"))
        output = output.stdout.decode("utf-8")
        absent_devices = []
    ...
    def is_net_device_active_in_peer(self):
        ...
        #for line in output.stdout.decode("utf-8"):
        for line in output:
            for intf in self.peer_intfs:
                if intf in line and 'ACTIVE' in line:
                    active_devices.append(intf)
    ...
    def ip_restore_peer(self):
        ...
        for ip, interface in zip(self.peer_ips, self.peer_intfs):
            if self.peer_distro in ['rhel', 'fedora']:
                ...
                self.session.cmd("echo \'%s\' > %s" % ('TYPE=Ethernet', file_name))
                self.session.cmd("echo \'%s\' >> %s" % ('BOOTPROTO=none', file_name))
                self.session.cmd("echo \'%s\' >> %s" % ('NAME=%s' % interface, file_name))
                self.session.cmd("echo \'%s\' >> %s" % (('DEVICE=%s' % interface), file_name))
                self.session.cmd("echo \'%s\' >> %s" % ('ONBOOT=yes', file_name))
                self.session.cmd("echo \'%s\' >> %s" % ('IPADDR=%s' % ip, file_name))
                self.session.cmd("echo \'%s\' >> %s" % ('NETMASK=%s' % self.netmask, file_name))
                self.session.cmd("echo \'%s\' >> %s" % ('IPV6INIT=yes', file_name))
                self.session.cmd("echo \'%s\' >> %s" % ('IPV6_AUTOCONF=yes', file_name))
                self.session.cmd("echo \'%s\' >> %s" % ('IPV6_DEFROUTE=yes', file_name))
