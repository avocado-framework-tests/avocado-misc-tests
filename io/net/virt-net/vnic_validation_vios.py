...
class VnicLifecycle(Test):
    ...
    def setUp(self):
        ...
        if not self.hmc_ip:
            self.log.info(
                "HMCIPAddr not found via lsrsrc IBM.MCP, "
                "falling back to lssrc -ls mcproxy for hostname")
            self.hmc_ip = self._get_hmc_from_mcproxy()
        if not self.hmc_ip:
            self.cancel(
                "Could not determine HMC IP via lsrsrc IBM.MCP, "
                "lssrc -ls mcproxy, or hmc_ip param")
        ...
        if not self.session_hmc.connect():
            self.fail("Failed to connect to HMC %s" % self.hmc_ip)
        ...
        if not wait.wait_for(self.session_vios.connect, timeout=30):
            self.fail("Failed to connect to VIOS %s" % self.vios_ip)
    ...
    def _get_hmc_from_mcproxy():
        ...
        Some distros do not have RSCT fully configured, causing lsrsrc to
        return empty output. This fallback covers those cases (see PR #3226).

        :returns: HMC hostname string, or '' if not found.
        """
        for line in process.system_output(
                'lssrc -ls mcproxy',
                ignore_status=True, shell=True, sudo=True
    ...
    def _verify_vios_mapping(self, iface, failures):
        """
    ...
    def _delete_and_verify_hostos(self, iface, failures):
        """
    ...
    def _verify_vios_clean(self, iface, failures):
        """
    ...
    def test_vnic_lifecycle(self):
        ...
        for iteration in range(1, self.num_iterations + 1):
            ...
            self.log.info("=" * 60)
            if iface:
                self._verify_vios_mapping(iface, failures)
            else:
                failures.append(
