...
class NetworkVirtualization(Test):
    ...
    def setUp(self):
        ...
        if '6x' not in str(self.name.name):
            ...
            if len(self.vios_name) != len(self.backing_adapter):
                self.cancel('Backing Device counts and vios name counts '
                            'differ')
        ...
        if '6x' not in str(self.name.name) and len(
                self.vnic_priority) != len(self.backing_adapter):
    ...
    def device_add_remove(self, slot, mac, sriov_port, adapter_id, operation):
        ...
        if operation == 'add':
            mac_attr = "mac_addr=%s," % mac if mac else ""
            cmd = 'chhwres -m %s --id %s -r virtualio --rsubtype vnic \
                   -o a -s %s -a \"auto_priority_failover=%s,%s%s\" '\
                   % (self.server, self.lpar_id, slot,
                      self.auto_failover, mac_attr, backing_device)
        else:
            cmd = 'chhwres -m %s --id %s -r virtualio --rsubtype vnic \
                   -o r -s %s'\
                   % (self.server, self.lpar_id, slot)
    ...
    def _vnic_add_ping_remove(self, slots, macs, adapter_ids, ports,
        ...
        for slot, mac, adapter_id, port, ip, netmask in zip(
            ...
            if not mac:
                cmd = ('lshwres -r virtualio -m %s --rsubtype vnic '
                       '--filter "lpar_names=%s,slots=%s" -F mac_addr'
                       % (self.server, self.lpar, slot))
                mac = self.session_hmc.cmd(
                    cmd).stdout_text.strip().replace(':', '')
            elif mac not in str(output):
    ...
    def _prepare_6x_params(self):
        '''
        Helper for 6x tests:
        - Auto-increments slots if 1 base slot provided
        - Round-robins adapters and ports
        - Resolves device_ips and peer_ips (fallback to device_ip / peer_ip)
        - Supports HMC-generated MACs if mac_id is omitted or empty
        '''
        target_count = 6

        # 1. Incremental Slot Numbers
        if len(self.slot_num) == 1:
            base_slot = int(self.slot_num[0])
            slots = [str(base_slot + i) for i in range(target_count)]
        elif len(self.slot_num) == target_count:
            slots = self.slot_num
        else:
            self.cancel("Provide either 1 starting slot or 6 slot numbers")

        # 2. Round-robin Adapters and Ports
        num_adapters = len(self.backing_adapter_id)
        adapter_ids = [self.backing_adapter_id[i % num_adapters]
                       for i in range(target_count)]
        num_ports = len(self.sriov_port)
        ports = [self.sriov_port[i % num_ports]
                 for i in range(target_count)]

        # 3. Read device_ips and peer_ips (fallback: device_ip / peer_ip)
        device_ips_param = self.params.get('device_ips', default=None)
        if device_ips_param:
            device_ips = device_ips_param.split(' ')
        else:
            device_ips = self.device_ip

        peer_ips_param = self.params.get('peer_ips', default=None)
        if peer_ips_param:
            peer_ips = peer_ips_param.split(' ')
        else:
            peer_ips = self.peer_ip

        if len(device_ips) < target_count or len(peer_ips) < target_count:
            self.cancel("6 entries required for device_ips (or device_ip) and "
                        "peer_ips (or peer_ip)")

        # 4. MAC addresses (HMC auto-generated if empty/omitted)
        mac_id_param = self.params.get('mac_id', default='')
        if mac_id_param:
            macs = [m.replace(':', '') for m in mac_id_param.split(' ')]
            if len(macs) < target_count:
                macs.extend([''] * (target_count - len(macs)))
        else:
            macs = [''] * target_count

        netmasks = [self.netmask[i % len(self.netmask)]
                    for i in range(target_count)]
    ...
    def test_vnic_6x_single_card(self):
        ...
        - Incremental slot numbers (starting slot or 6 explicit slots)
        - Single SR-IOV card with ports round-robined
        - HMC auto-generated MACs if mac_id omitted
        - Uses device_ips / peer_ips (6 entries)

        YAML: network_virtualization.yaml
        Required keys: slot_num, sriov_adapters (1 card), sriov_ports,
          vios_names, device_ips (or device_ip, 6 entries),
          peer_ips (or peer_ip, 6 entries), netmasks, bandwidth, auto_failover
    ...
    def test_vnic_6x_multi_card(self):
        ...
        - Requires at least 2 distinct adapters in sriov_adapters;
          cancels otherwise.
        - Incremental slot numbers (starting slot or 6 explicit slots)
        - Round-robin across distinct adapters and ports
        - HMC auto-generated MACs if mac_id omitted
        - Uses device_ips / peer_ips (6 entries)

        YAML: network_virtualization.yaml
        Required keys: slot_num, sriov_adapters (>=2 distinct cards),
          sriov_ports, vios_names, device_ips (or device_ip, 6 entries),
          peer_ips (or peer_ip, 6 entries), netmasks, bandwidth, auto_failover

        :avocado: tags=net,vnic,dlpar,privileged,power
        '''
        distinct_adapters = list(dict.fromkeys(self.backing_adapter))
        if len(distinct_adapters) < 2:
            self.cancel("test_vnic_6x_multi_card requires at least 2 distinct "
                        "SR-IOV cards in 'sriov_adapters', but got %d: %s"
                        % (len(distinct_adapters), distinct_adapters))
        slots, macs, adapter_ids, ports, device_ips, netmasks, peer_ips = \
            self._prepare_6x_params()
        self._vnic_add_ping_remove(
            slots, macs, adapter_ids, ports, device_ips, netmasks, peer_ips)
