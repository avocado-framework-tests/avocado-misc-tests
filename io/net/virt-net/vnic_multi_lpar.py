...
class VnicMultiLpar(Test):
    ...
    def setUp(self):
        ...
        self.hmc_ip = wait.wait_for(self._get_hmc_ip, timeout=30)
        if not self.hmc_ip:
    ...
    def _get_hmc_ip():
        '''
        Resolve the HMC IP/hostname.
        First probes IBM.MCP HMCIPAddr via lsrsrc (RSCT); if that returns
        nothing, falls back to parsing the Hostname field from lssrc -ls
        mcproxy.  Returns the address string, or '' if both sources fail.
        '''
        for line in process.system_output(
                'lsrsrc IBM.MCP HMCIPAddr',
                ignore_status=True, shell=True,
                sudo=True).decode("utf-8").splitlines():
            if 'HMCIPAddr' in line:
    ...
    def _check_peer_rsct(self):
        '''Verify RSCT services are running on LPAR2 (peer).

        Runs ``lssrc -a`` over the existing SSH session and cancels the
        test if any subsystem is reported as ``inoperative``.  DLPAR
        hot-add/remove requires a live RMC connection which depends on
        RSCT being healthy on the remote partition.
        '''
        if self.session_lpar2 is None:
            return
        out = self.session_lpar2.cmd(
            'lssrc -a', ignore_status=True).stdout_text
        inoperative = [line.strip() for line in out.splitlines()
                       if 'inoperative' in line.lower()]
        if inoperative:
            self.cancel(
                "RSCT subsystems inoperative on LPAR2 (%s): %s"
                % (self.lpar2_ip, '; '.join(inoperative)))
        self.log.info("LPAR2: all RSCT subsystems are active")

    def _lpar_vnic_add_safe(self, lpar_id, lpar_name, slot, mac,
                            sriov_port, priority="1"):
        '''Add a vNIC to the specified LPAR, removing any stale vNIC on
        the same slot first.

        If ``lshwres`` already shows a vNIC in *slot* for *lpar_name*,
        the existing entry is removed before the new add is issued.
        This prevents a ``chhwres`` failure caused by a leftover vNIC
        from a previous interrupted test run.
        '''
        if self._lpar_vnic_listed(lpar_name, slot):
            self.log.warning(
                "Stale vNIC found on %s slot %s — removing before add",
                lpar_name, slot)
            self._lpar_vnic_remove(lpar_id, slot)
            # Give HMC a moment to complete the remove before re-adding.
            time.sleep(5)
        self._lpar_vnic_add(lpar_id, slot, mac, sriov_port, priority)
    ...
    def test_multi_lpar_same_adapter(self):
        ...
        # Verify RSCT is healthy on the peer before any DLPAR operation.
        self._check_peer_rsct()

        try:
            # ── Phase 1: add vNIC to LPAR1 ───────────────────────────────────
            self.log.info("Adding vNIC to LPAR1 slot=%s mac=%s port=%s",
                          self.lpar1_slot, self.lpar1_mac, sriov_port)
            self._lpar_vnic_add_safe(self.lpar1_id, self.lpar1,
                                     self.lpar1_slot, self.lpar1_mac,
                                     sriov_port)
            ...
            self._lpar_vnic_add_safe(self.lpar2_id, self.lpar2,
                                     self.lpar2_slot, self.lpar2_mac,
                                     sriov_port)
