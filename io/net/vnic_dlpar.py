...
class Vnic(Test):
    ...
    def setUp(self):
        ...
        self.count = self.params.get("count", '*', default=None)
...
#            self.fail("Command startsrc -g rsct_rm failed")
        ...
        try:
            for rsct_service in ["startsrc -g rsct", "startsrc -g rsct_rm"]:
                process.run(rsct_service, shell=True, sudo=True)
        except CmdError as details:
            self.log.debug(str(details))
            self.fail("Command %s failed", rsct_service)
