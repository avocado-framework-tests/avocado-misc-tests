...
class pingpong(Test):
    '''
       ib_pingpong test
       pingpong tools should be installed
    '''
    def setUp(self):
        '''
        ...
        self.flag = self.params.get("ext_flag")
    ...
    def pingpong_exec(self, arg1, arg2, arg3):
        ...
        ret = process.system(cmd, shell=True)
        if ret != 0:
        ...
        tmp = "timeout %s %s -d %s -g %d -i %d %s %s %s" \
            % (self.to, arg1, self.CA, self.GID, self.PORT, self.PEER_IP,
                test, arg3)
        ret = process.system(tmp, shell=True)
        if ret != 0:
    ...
    def test_ib_pingpong(self):
        ...
        val1 = ""
        val2 = ""
        test_op = self.params.get("test_opt", default="")
        test_opt = test_op.split(",")
        for val in test_opt:
            try:
                val1, val2 = val.split()
            except:
        ...
        ext_test_op = self.params.get("ext_test_opt", default="")
        ext_test_opt = ext_test_op.split(",")
        if self.flag == "1":
            for val in ext_test_opt:
