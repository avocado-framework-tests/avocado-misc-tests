...
class MultiportStress(Test):
    ...
    def setUp(self):
        ...
        self.packet_size = self.params.get("packet_size")
    ...
    def test_multiport_stress(self):
        ...
        for host, peer in map(None, self.host_interfaces, self.peer_ips):
            ...
            cmd = "ping -I %s %s -c %s" % (host, peer, self.packet_size)
            print cmd
        ...
        for proc in parallel_procs:
            ...
            out_buf = out_buf.split('\n')
            for val in out_buf:
                if 'loss' in val:
                    if '0% packet loss' in val:
                        self.log.info('Ping test passed')
                    else:
