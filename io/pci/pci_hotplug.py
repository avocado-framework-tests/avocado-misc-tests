...
class PCIHotPlugTest(Test):
    ...
    def test_pcihotplug_with_lockdown(self):
        ...
        self.log.info(f"Current lockdown state: {original_state}")
        if self.lockdown_enable is True:
            if original_state == "integrity" or original_state == "confidentiality":
                self.cancel("lockdown mode already set to %s" % original_state)
