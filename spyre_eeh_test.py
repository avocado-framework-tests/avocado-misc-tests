...
class SpyreEEHTest(Test):
    ...
    def setUp(self):
        ...
        # Check if running on PowerPC platform
        ...
        # Get PCI addresses from YAML (space-separated string)
        ...
        # Parse space-separated PCI addresses
        ...
        # Get max freeze count from YAML (default: 5)
    ...
    def _inject_eeh_error(self, pci_address):
        ...
        # Inject error by writing PCI address to eeh_dev_break
    ...
    def _validate_eeh_in_dmesg(self, pci_address, timeout=30):
        ...
        # Pattern to match: "vfio-pci 0382:60:00.0: Going to break:"
        pattern = rf"vfio-pci\s+{re.escape(pci_address)}:\s+Going to break:"

        start_time = time.time()
        while time.time() - start_time < timeout:
            try:
                # Get new dmesg entries
                current_dmesg = dmesg.collect_dmesg()
                
                # Search for the pattern in dmesg
    ...
    def _validate_pci_device_present(self, pci_address):
        ...
        try:
            # Use lspci to check if device is present
            # Format: lspci -s 0382:60:00.0
    ...
    def _validate_container_restarted(self, container_name, timeout=60):
        ...
        # Wait for container to restart (new ID or status change)
        start_time = time.time()
        while time.time() - start_time < timeout:
            time.sleep(5)
    ...
    def test_eeh_inject_single_pci(self):
        ...
        # Step 4: Validate container restart (if configured)
    ...
    def test_eeh_inject_all_pci(self):
        ...
        self.log.info("")
        self.log.info("=" * 80)
        self.log.info("EEH Injection Summary")
        self.log.info("=" * 80)
    ...
    def tearDown(self):
        ...
        """
        self.log.info("=" * 80)
        self.log.info("Spyre EEH Test - Teardown")
        self.log.info("=" * 80)

        # Check for any new kernel errors
        try:
            current_dmesg = dmesg.collect_dmesg()
            # Look for critical errors that appeared during test
            error_patterns = [
                r"kernel panic",
                r"Oops:",
                r"BUG:",
