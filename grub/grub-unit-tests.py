...
"""
Build and run the GNU GRUB unit/shell/filesystem test suite on PowerPC.

Fetches the distro GRUB2 source, compiles it for the powerpc-ieee1275
platform, and drives ``make check`` -- skipping tests that are not
applicable on ppc64le.
"""
...
class GrubUnitTest(Test):
    ...
    _NOT_PPC_TESTS = {
        'ahci_test':
            'AHCI not present on powerpc-ieee1275 QEMU',
        'ehci_test':
            'EHCI not present on powerpc-ieee1275 QEMU',
        'ohci_test':
            'OHCI not present on powerpc-ieee1275 QEMU',
        'uhci_test':
            'UHCI not present on powerpc-ieee1275 QEMU',
        'pata_test':
            'no ATA/PATA driver for powerpc-ieee1275',
        'fddboot_test':
            'powerpc-ieee1275 does not support floppy boot',
        'serial_test':
            'ieee1275 uses OpenFirmware console, not PCI serial',
        'partmap_test':
            'exits 77 detecting live OF disk on powerpc-ieee1275',
        'netboot_test':
            'PXE netboot not supported on powerpc-ieee1275 QEMU',
        'tpm2_key_protector_test':
            'requires grub-emu; powerpc-ieee1275 uses qemu-system-ppc',
        'minixfs_test':
            'minix kernel module absent on ppc64le',
        'erofs_test':
            'erofs UUID endian mismatch on ppc64le (blkid vs GRUB)',
        'grub_func_test':
            'videotest_checksum pixel values are x86-specific',
        'grub_cmd_cryptomount':
            'QEMU mac99 single IDE bus conflict with grub.iso',
        # Filesystem tests that require tools absent on ppc64le distros
        'hfs_test':
            'mkfs.hfs (hfsprogs) not available on ppc64le',
        'hfsplus_test':
            'mkfs.hfsplus (hfsprogs) not available on ppc64le',
        'reiserfs_test':
            'mkfs.reiserfs (reiserfsprogs) not available on ppc64le',
        'btrfs_test':
            'mkfs.btrfs (btrfs-progs) not available on ppc64le RHEL/SLES CI',
        'f2fs_test':
            'mkfs.f2fs (f2fs-tools) not available on ppc64le',
        'nilfs2_test':
            'mkfs.nilfs2 (nilfs-utils) not available on ppc64le',
        'zfs_test':
            'ZFS kernel module not available on ppc64le RHEL/SLES',
        'zfs_zstd_test':
            'ZFS kernel module not available on ppc64le RHEL/SLES',
        'jfs_test':
            'mkfs.jfs (jfsutils) not available on ppc64le RHEL',
        'romfs_test':
            'genromfs not available on ppc64le RHEL/SLES',
    }
    ...
    _PARALLEL_FS_TESTS = [
        ('fat_test',      ['vfat16a', 'vfat12a', 'vfat12', 'vfat16',
                           'vfat32']),
        ('squashfs_test', ['squash4_gzip', 'squash4_xz', 'squash4_lzo']),
        ('ext234_test',   ['ext2_old', 'ext2', 'ext3', 'ext4',
                           'ext4_metabg', 'ext4_encrypt']),
    ...
    def setUp(self):
        ...
        if 'ppc' not in platform.machine():
            self.cancel(
                f'Test requires PowerPC (detected: {platform.machine()})')
        ...
        is_suse = (
            'SuSE' in detected_distro.name or
            'suse' in (detected_distro.name or '').lower()
        )
        rhel_ver = int(
            str(detected_distro.version or '0').split('.', maxsplit=1)[0])

        qemu_ok = self._qemu_present()
        deps = ['gcc', 'gcc-c++', 'make', 'automake', 'autoconf',
                'bison', 'flex', 'python3', 'help2man', 'rsync', 'patch',
                'parted', 'device-mapper-devel', 'xz-devel', 'fuse-devel']
        if is_suse:
            ...
            if self.test_type in ('shell', 'fs') and not qemu_ok:
                deps += ['ninja', 'meson', 'glib2-devel',
                         'libpixman-1-0-devel', 'zlib-devel',
                         'libslirp-devel']
            ...
            if self.test_type in ('shell', 'fs') and not qemu_ok:
                deps += ['ninja-build', 'meson', 'glib2-devel',
                         'pixman-devel', 'zlib-devel', 'libslirp-devel']
                if rhel_ver < 10:
        ...
        try:
            ...
            if os.path.exists(max_loop_path):
                with open(max_loop_path, encoding='utf-8') as fh:
                    cur = int(fh.read().strip())
                if cur < 64:
                    process.run(
                        f'modprobe loop max_loop=64 || '
                        f'echo 64 > {max_loop_path}',
        ...
        # Attempt a full upstream build; fall back to a pre-built cache on
        # any network/build failure (e.g. Savannah gnulib HTTP 500 during
        # bootstrap).
        ...
        try:
            ...
            build_error = exc
            self.log.warning(
                'upstream build failed (%s); trying cache at %s',
                exc, cache_dir)
        ...
        if built_from_source:
            ...
            try:
                ...
                self.log.info(
                    'Updated grub build cache at %s', cache_dir)
            ...
            if not os.path.isdir(cache_dir):
                self.cancel(
                    f'upstream build failed ({build_error}) and no '
                    f'cache at {cache_dir}')
            self.log.info(
                'Using cached GRUB build from %s', cache_dir)
            # Clean any partial clone/build before rsyncing.
            if os.path.isdir(self.srcdir):
                shutil.rmtree(self.srcdir)
            if process.run(
                    f'rsync -a {cache_dir}/ {self.srcdir}/',
    ...
    def _patch_test_parallel(self, script_name, subtests):
        sentinel = f'_{script_name}_parallel_patched'
        parallel_body = (
            f'# {sentinel}\n_pids=\nfor _sub in {" ".join(subtests)}; do\n'
        ...
        in_file = os.path.join(
            self.srcdir, 'tests', f'{script_name}.in')
        if os.path.isfile(in_file):
            old = ''.join(
                f'"@builddir@/grub-fs-tester" {s}\n' for s in subtests)
            with open(in_file, encoding='utf-8') as fh:
                already = sentinel in fh.read()
            if not already and self._patch_file(
                    in_file, old, parallel_body % '@builddir@'):
                process.run(
                    f'cd {self.srcdir} && make {script_name}',
                    shell=True, ignore_status=True)
                return
    ...
    def _qemu_present():
        return (
            process.system('which qemu-system-ppc',
                           ignore_status=True) == 0 and
            process.system('which qemu-system-ppc64',
    ...
    def _build_from_tarball(self, name, src, configure_args,
                            pre_configure=None):
        ...
        if pre_configure:
            if process.run(pre_configure, ignore_status=True).exit_status:
                self.fail(f'{name} {pre_configure} failed')
        if process.run(f'./configure {configure_args}',
                       ignore_status=True).exit_status:
            self.fail(f'{name} configure failed')
        if build.make(src, extra_args=f'-j{ncpus}',
                      process_kwargs={'ignore_status': True}):
            self.fail(f'{name} make failed')
        if process.system('make install', ignore_status=True):
            self.fail(f'{name} make install failed')

    def _fetch_and_build(self, name, url, version, prefix,
                         pre_configure=None):
        """Fetch a tarball asset, extract it, build and install it."""
        tarball = self.fetch_asset(url, expire='30d')
        src = os.path.join(self.workdir, f'{name}-{version}')
    ...
    def _ensure_qemu(self):
        ...
        src = os.path.join(self.workdir, f'qemu-{qemu_version}')
        archive.extract(tarball, self.workdir)
    ...
    def _ensure_erofs_utils(self):
        ...
        self._fetch_and_build(
            ...
            self.params.get('erofs_utils_url', default=(
                'https://github.com/erofs/erofs-utils/archive/'
                'refs/tags/v1.8.1.tar.gz')),
    ...
    def _ensure_ntfs3g(self):
        ...
        self._fetch_and_build(
            ...
            self.params.get('ntfs3g_url', default=(
                'https://github.com/tuxera/ntfs-3g/archive/'
                'refs/tags/2022.10.3.tar.gz')),
    ...
    def _check_kernel_modules(self):
        for mod in ('minix', 'f2fs'):
            if process.run(
                ...
                self.log.warning(
                    '%s module absent; %s_test will SKIP', mod, mod)
    ...
    def test(self):
        ...
        ncpus = os.cpu_count() or 1
        suite_timeout = int(
            self.params.get('suite_timeout', default='7200') or '7200')
        slow_timeout = int(
            self.params.get('slow_test_timeout', default='60') or '60')

        slow_tests = [
            (name, slow_timeout if default_t == 0 else default_t, is_skip)
        ...
        ppc_skip = {n: r for n, r in self._NOT_PPC_TESTS.items()
                    if n in all_tests}
        for name, reason in ppc_skip.items():
            self.log.info(
                'SKIP [not-ppc]: %s -- not related to powerpc -- %s',
                name, reason)
        ...
        fast_out = process.run(
            f'setsid timeout {suite_timeout} make check '
            f'-j{ncpus} -k TESTS="{" ".join(fast_tests)}" '
            f'</dev/null 2>&1',
            shell=True, env=env, ignore_status=True)
        ...
        def _run_slow(name, timeout, skip_on_timeout):
            log = os.path.join(
                self.srcdir, f'test-suite-{name}.log')
            ret = subprocess.call(
                f'setsid timeout {timeout} make check '
                f'TESTS="{name}" </dev/null >{log} 2>&1',
                shell=True, env=env, cwd=self.srcdir,
                stdin=subprocess.DEVNULL)
            try:
                with open(log, errors='replace', encoding='utf-8') as fh:
                    text = fh.read()
            except OSError:
                text = ''
            if ret == 124:
        ...
        with lock:
            for name, _, skip_on_timeout in slow_tests:
                if name not in slow_results:
                    slow_results[name] = (
                        'SKIP' if skip_on_timeout else 'TIMEOUT')
        ...
        self.log.info(sep)
        self.log.info(
            'GRUB test results [test_type=%s]', self.test_type)
        self.log.info(sep)
        ...
        for name, reason in sorted(ppc_skip.items()):
            self.log.info(
                '  SKIP [not-ppc]: %-30s (%s)', name, reason)
        ...
        if fail_l or timeout_l:
            self.fail(
                f'{len(fail_l)} FAILED  {len(timeout_l)} TIMEOUT')
