...
"""
DLPAR operations with lockdown feature check
"""
import time
import subprocess
import sys
...
# Configure logging for lockdown checks
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)
 
def check_lockdown_status():
    """Check if DLPAR lockdown is enabled"""
    try:
        result = subprocess.run(['lsattr', '-El', 'sys0', '-a', 'dlpar_mem_lck'],
                              capture_output=True, text=True, check=True)
        output = result.stdout.strip()
        if 'enable' in output.lower() or '1' in output:
            logger.info("✓ DLPAR lockdown is ENABLED")
            return True
        logger.warning("✗ DLPAR lockdown is NOT ENABLED")
        return False
...
def enable_lockdown():
    """Enable DLPAR lockdown"""
    try:
        logger.info("Enabling DLPAR lockdown...")
        subprocess.run(['chdev', '-l', 'sys0', '-a', 'dlpar_mem_lck=enable'],
                      capture_output=True, text=True, check=True)
        logger.info("✓ DLPAR lockdown enabled successfully")
        return True
    except Exception as e:
        logger.error(f"Failed to enable lockdown: {e}")
        return False
 
def inform_user_to_reboot():
    """Inform user to reboot manually"""
    message = """
╔════════════════════════════════════════════════════════════════════════════╗
║                          ⚠️  REBOOT REQUIRED  ⚠️                            ║
╠════════════════════════════════════════════════════════════════════════════╣
║  DLPAR lockdown has been enabled.                                          ║
║  Please REBOOT the system manually, then run this script again.            ║
║                                                                            ║
║  To reboot: sudo shutdown -r now  OR  sudo reboot                          ║
╚════════════════════════════════════════════════════════════════════════════╝
"""
    print(message)
    logger.warning("User must reboot before running DLPAR tests")
 
def check_dlpar_lockdown():
    """
    Main lockdown check function - runs before DLPAR tests
    Returns True if lockdown is enabled and tests can proceed
    Returns False if lockdown was just enabled (needs reboot)
    """
    logger.info("=" * 60)
    logger.info("STEP 1: Checking DLPAR lockdown status...")
    logger.info("=" * 60)
   
    is_enabled = check_lockdown_status()
   
    if not is_enabled:
        logger.info("\nSTEP 2: Lockdown not enabled. Enabling now...")
        if enable_lockdown():
            logger.info("\nSTEP 3: Informing user to reboot...")
            inform_user_to_reboot()
            return False  # Exit - user needs to reboot
        else:
            logger.error("Failed to enable lockdown")
            sys.exit(1)
    else:
        logger.info("\nSTEP 2: Lockdown already enabled")
        logger.info("\nSTEP 3: Proceeding with DLPAR tests...")
        logger.info("=" * 60)
...
class DlparPci(Test):
    ...
    def setUp(self):
        ...
        Test all services.
        '''
        # LOCKDOWN CHECK - Must pass before proceeding with tests
        if not check_dlpar_lockdown():
            self.cancel("DLPAR lockdown was just enabled. Please reboot and run again.")
