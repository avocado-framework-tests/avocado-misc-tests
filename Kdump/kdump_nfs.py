...
class KDUMP(Test):
    ...
    def setUp(self):
        ...
        self.ip = self.params.get('ip', default='9.40.193.224')
        try:
    ...
    def configure_nfs(self):
        ...
        if self.distro == "Ubuntu":
            ...
            if self.run_cmd_out("cat %s | grep -Eai 'Not ready to'" % log_file_2):
        ...
        if self.distro == "rhel":
            ...
            if self.run_cmd_out("cat %s | grep -Eai 'Kdump is not operational'" % log_file_2):
                self.fail("Kdump is not operational after configuring nfs")
            else:
                self.log.info("Kdump status is operational after configuring nfs")
