...
class Xfstests(Test):
    ...
    def _get_tests_for_group(self, group):
        ...
        '''
        tests = []
        for g_test in open(os.path.join(self.srcdir, 'group')).readlines():
            match = self.GROUP_TEST_LINE_RE.match(g_test)
            if match is not None:
                test = match.groups()[0]
                groups = match.groups()[1]
                print "int this test group"
                print test
                print groups
    ...
    def _parse_test_range(self, test_range):
    ...
    def test(self):
        ...
        if self._test_range:
            test_list = self._parse_test_range(self._test_range)
