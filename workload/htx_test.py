...
class HtxTest(Test):
    ...
    def _nic_setup(self):
        ...
        if not self.session.connect():
            self.error('Failed connecting to peer %s' % self.peer_ip)
    ...
    def _build_htx_nic(self):
        ...
        for pattern in [host_pattern, peer_pattern]:
            ...
            if process.system(cmd, shell=True, ignore_status=True):
                self.fail('Failed to download HTX RPM: %s' % rpm_url)
            ...
            if host_pattern == peer_pattern:
                ...
                if not self.session.copy_files(
                    ...
                    self.error('Failed to copy RPM to peer: %s' % latest_rpm)
                output = self.session.cmd(
            ...
            if pattern == peer_pattern:
                ...
                output = self.session.cmd(
                    'rpm -ivh --nodeps /tmp/%s --force' % latest_rpm)
                if output.exit_status != 0:
                    self.fail('RPM install failed on peer: %s' % latest_rpm)
