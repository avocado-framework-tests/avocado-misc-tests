...
void write_memory(void *addr, unsigned long pattern_64, unsigned long size)
{
	...
	for(chunk_offset = 0; chunk_offset < size; chunk_offset += CHUNK_SIZE) {
		current_chunk_size = ((size - chunk_offset) > CHUNK_SIZE) ? CHUNK_SIZE : (size - chunk_offset);
...
{
	unsigned long psize, npages, iterator;
	unsigned long base_offset;
	uint64_t pfn_buf[PAGEMAP_BATCH];
	ssize_t bytes_read;
	...
	psize = getpagesize();
	npages = memory / psize;
	/* Offset in pagemap for the first page of this mapping */
	base_offset = ((unsigned long)addr / psize) * PMAP_SIZE;
	...
	for (iterator = 0; iterator < npages; iterator += PAGEMAP_BATCH) {
		batch = npages - iterator;
		if (batch > PAGEMAP_BATCH)
			batch = PAGEMAP_BATCH;

		bytes_read = pread(fd, pfn_buf, batch * PMAP_SIZE,
				   base_offset + iterator * PMAP_SIZE);
		if (bytes_read != (ssize_t)(batch * PMAP_SIZE)) {
			perror("pread() failed");
			close(fd);
			exit(-1);
		}
...
void write_read_pattern_into_memory()
{
	...
	unsigned long pattern_64 = (PATTERN << 32) | PATTERN;
