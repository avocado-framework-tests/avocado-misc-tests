...
class BCCTest(Test):
    ...
    def setUp(self):
        ...
        self.is_sles = (
            "sles" in self.distro_name.lower()
            or self.distro_name == 'SuSE'
        )
        ...
        if self.is_rhel:
            deps = [
                ...
                # BCC specific build dependencies
                "bison",
                "bpftool",
                "clang-devel",
                "cmake",
        ...
        if failed_packages:
            self.log.warning(
                "Some packages failed to install: %s"
    ...
    def download_bcc_source(self):
        ...
        # Distribution-specific download command
        if self.is_rhel:
            cmd = "dnf --source download bcc"
        else:  # SLES
            # For SLES, we need to use zypper to download source
    ...
    def build_bcc(self):
        ...
        # Install build dependencies - distribution specific
    ...
    def run_bcc_tests(self, build_dir):
        ...
        # Use the first (and likely only) BCC directory
