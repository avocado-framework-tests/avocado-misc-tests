...
class SpyreHostConfig(Test):
    ...
    def test_huggingface_login(self):
        ...
        if not self.run_cmd(f"echo '{self.hf_token}' | hf auth login --token-stdin"):
            self.fail(
    ...
    def test_configure_ibm_repo(self):
        ...
        if self.servicereport_rpm_url:
            ...
            if not RpmBackend.rpm_install(rpm_path, no_dependencies=True, replace=False):
                self.cancel("Installation of servicereport rpm failed")

            # Verify installation
            if not RpmBackend.rpm_verify(os.path.basename(rpm_path)):
                self.fail(
    ...
    def test_podman_login(self):
        ...
        if not self.run_cmd(f"podman login -u iamapikey -p '{self.api_key}' {self.registry}"):
            self.fail(
                ...
                f"and that the system can reach {self.registry}, then retry."
            )
    ...
    def test_create_user(self):
        ...
        if self.password:
            ...
            if not self.run_cmd(f"printf '%s:%s' '{self.username}' '{self.password}' | chpasswd"):
                self.fail(
    ...
    def test_create_model_directories(self):
        ...
        dirs = [
            ...
            {"path": f"{self.models_dir}", "mode": "0775",
