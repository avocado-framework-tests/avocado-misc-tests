...
/* Global local to serialize records aggregation */
...
search_mem(void)
{
	...
	unsigned int state = 0x0c0ffee0;
        unsigned int sleep_at_part;
        unsigned int sleep_interval_part;
        int hvnt_slept = 1;
        struct timeval tv;
        gettimeofday(&tv,NULL);
        
        srand(tv.tv_usec);
        sleep_at_part=1 + (int)( (double)sleep_at * rand() / (double)( RAND_MAX + 1.0 ) );
        sleep_interval_part=(int)( (double)sleep_interval * (double)rand() / (double)( RAND_MAX + 1.0 ) );
        if(verbose){
                printf("Seed: %ld\n",tv.tv_usec);
                printf("Sleep At: %d Sleep At Part: %d\n",sleep_at,sleep_at_part);
                printf("Sleep Interval: %d Sleep Interval Part: %d\n",sleep_interval,sleep_interval_part);
        }
	...
	for (i = 0; threads_go == 1; i++) {
		...
		//	usleep(sleep_interval);
                if(sleep_at && !(i % sleep_at_part) && hvnt_slept){
                        usleep(sleep_interval_part);
                        hvnt_slept=0;
                }
                if(sleep_at && !(i % sleep_at)){
                        usleep(sleep_interval-sleep_interval_part);
                        hvnt_slept=1;
                }
	}
...
thread_run(void *arg)
{
	...
	if (verbose > 1)
		printf("[%lx]Thread finished, %f seconds %d records\n",
		       pthread_self(), difftime(time(NULL), start_time), records_local);
