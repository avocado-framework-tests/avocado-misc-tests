...
class SchedulerTunablesTest(Test):
    ...
    def setUp(self):
        ...
        if configured_workers == 0 or configured_workers is None:
        ...
        if not os.path.ismount('/sys/kernel/debug'):
            ...
            try:
                ...
                if result.exit_status != 0:
                    self.log.warning(
                        "Failed to mount debugfs. Some tunables may not be \
                                available.")
                else:
                    self.log.info("Successfully mounted debugfs")
            except Exception as e:
                self.log.warning(
                    "Could not mount debugfs: %s. Some tunables may not \
                            be available.", str(e))
        else:
            ...
            if 'context-switches' in line:
                ...
                for i, part in enumerate(parts):
                    if 'migrations/sec' in part and i > 0:
                        metrics['migrations_per_second'] = float(parts[i-1])
    ...
    def test_01_baseline(self):
        ...
        workload_type = self.params.get(
            'tests.baseline.workload_type', default='mixed')
    ...
    def test_02_low_latency(self):
        ...
        if baseline_base_slice and baseline_migration_cost:
            ...
            workload_type = self.params.get(
                'tests.low_latency.workload_type', default='cpu')
            base_slice_factor = float(self.params.get(
                'tests.low_latency.tunables.base_slice_ns_factor',
                default=0.25))
            migration_cost_factor = float(self.params.get(
                'tests.low_latency.tunables.migration_cost_ns_factor',
                default=0.10))
    ...
    def test_03_high_throughput(self):
        ...
        if baseline_base_slice and baseline_migration_cost:
            ...
            workload_type = self.params.get(
                'tests.high_throughput.workload_type',
                default='context-switch')
            base_slice_factor = float(self.params.get(
                'tests.high_throughput.tunables.base_slice_ns_factor',
                default=3.0))
            migration_cost_factor = float(self.params.get(
                'tests.high_throughput.tunables.migration_cost_ns_factor',
                default=2.0))
    ...
    def test_04_migration_behavior(self):
        ...
        if baseline_nr_migrate:
            workload_type = self.params.get(
                'tests.migration_behavior.workload_type', default='fork')
            nr_migrate_factor = float(self.params.get(
                'tests.migration_behavior.tunables.nr_migrate_factor',
                default=2.0))
            ...
            if result['success'] and hasattr(self, 'baseline_metrics'):
                ...
                self._compare_and_log_metrics(
                    self.baseline_metrics, result['metrics'],
                    'cpu_migrations', 'CPU Migrations')
