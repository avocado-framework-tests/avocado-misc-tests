...
class SchedulerTunablesTest(Test):
        ...
        if not os.path.ismount('/sys/kernel/debug'):
            ...
            try:
                ...
                if result.exit_status != 0:
                    self.log.warning(
                        "Failed to mount debugfs. Some tunables under "
                        "/sys/kernel/debug/sched/ may not be available. "
                        "Tests will use procfs tunables where possible.")
                else:
                    self.log.info("Successfully mounted debugfs")
            except Exception as e:
                self.log.warning(
                    "Could not mount debugfs: %s. Some tunables under "
                    "/sys/kernel/debug/sched/ may not be available. "
                    "Tests will use procfs tunables where possible.",
                    str(e))
        else:
    ...
    def _get_tunable_path(self, tunable_name):
        ...
        Kernel version note for base_slice_ns:
          - kernel >= 6.6: /sys/kernel/debug/sched/base_slice_ns
          - kernel  < 6.6: /sys/kernel/debug/sched/latency_ns is the
            equivalent tunable (controls the CFS scheduling period/latency).
            /proc/sys/kernel/sched_latency_ns also exists on some distros.
        """
        tunable_paths = {
            'base_slice_ns': [
    ...
    def _check_and_save_tunables(self):
        ...
        For base_slice_ns: on kernels < 6.6 the equivalent tunable is
        latency_ns (same sysctl path family). _get_tunable_path() resolves
        this transparently, so we log an alias note when the fallback is used.
        """
        self.log.info("\n--- Checking Top 4 Scheduler Tunables ---")

        # Map of canonical name -> fallback display name for logging
        base_slice_fallbacks = {
            '/sys/kernel/debug/sched/latency_ns': 'latency_ns',
            '/proc/sys/kernel/sched_latency_ns': 'sched_latency_ns',
        }
        ...
        for tunable_name in tunables:
            ...
            if tunable_path:
                try:
                    ...
                    alias = base_slice_fallbacks.get(tunable_path)
                    if alias:
                        self.log.info(
                            "  ✓ %s (using %s on this kernel) = %s",
                            tunable_name, alias, value)
                    else:
                        self.log.info(
                            "  ✓ %s = %s", tunable_name, value)
                except Exception as e:
                    self.log.warning(
                        "  ⚠ Failed to read %s: %s", tunable_name, str(e))
            else:
                self.log.info(
                    "  - %s NOT FOUND (not supported on this kernel, "
                    "skipping)", tunable_name)
    ...
    def _get_tunable_display_name(self, tunable_name):
        """
        Return a display name for a tunable that reflects the actual path
        in use on this kernel. Useful for log messages when a fallback path
        (e.g. latency_ns instead of base_slice_ns) is being used.
        """
        fallback_labels = {
            '/sys/kernel/debug/sched/latency_ns': 'base_slice_ns(latency_ns)',
            '/proc/sys/kernel/sched_latency_ns':
            'base_slice_ns(sched_latency_ns)',
        }
        path = self._get_tunable_path(tunable_name)
        if path and tunable_name == 'base_slice_ns':
            return fallback_labels.get(path, tunable_name)
        return tunable_name
    ...
    def test_02_low_latency(self):
        ...
        Note: On kernels < 6.6, base_slice_ns is exposed as latency_ns under
        /sys/kernel/debug/sched/. _get_tunable('base_slice_ns') resolves this
        transparently via _get_tunable_path() fallback list.
        """
        self.log.info("\n" + "=" * 70)
        ...
        baseline_base_slice = self._get_tunable('base_slice_ns')
        baseline_migration_cost = self._get_tunable('sched_migration_cost_ns')
        base_slice_label = self._get_tunable_display_name('base_slice_ns')
        ...
        if baseline_base_slice and baseline_migration_cost:
            ...
            self._log_tunable_change(base_slice_label,
            ...
            if test_result['success']:
                ...
                TOLERANCE = 5.0  # ±5% deviation accepted as within noise
                if cs_change is not None:
                    if cs_change > TOLERANCE:
                        self.log.info(
                            '  ✓ EXPECTED: More context switches with '
                            'smaller time slices (+%.1f%%)', cs_change)
                    elif abs(cs_change) <= TOLERANCE:
                        self.log.info(
                            '  ✓ WITHIN TOLERANCE: Context switch change '
                            '%.1f%% is within ±%.1f%% noise threshold',
                            cs_change, TOLERANCE)
                    else:
                        # cs_change < -TOLERANCE: reduction > 5%
                        # On CFS (kernel < 6.6), reducing latency_ns with a
                        # pure CPU-bound workload can legitimately reduce
                        # context switches while increasing CPU migrations.
                        # Both observations confirm the tunable is active.
                        self.log.info(
                            '  ℹ NOTE: Fewer context switches observed '
                            '(%.1f%%) — normal for CPU-bound workloads with '
                            'reduced latency_ns; check CPU migrations for '
                            'tunable effect', cs_change)
            ...
            self.cancel(
                "Required tunables (base_slice_ns/latency_ns and "
                "sched_migration_cost_ns) not available on this kernel")
                ...
                if cs_change is not None:
                    if cs_change < -TOLERANCE:
                        self.log.info(
                            ...
                            'larger time slices (%.1f%%)', cs_change)
                    elif abs(cs_change) <= TOLERANCE:
                        self.log.info(
                            '  ✓ WITHIN TOLERANCE: Context switch change '
                            '%.1f%% is within ±%.1f%% noise threshold',
                        ...
                        # cs_change > +TOLERANCE: increase > 5%
                        # For switch-intensive workloads the sheer volume
                        # (~117M cs/10s) can dominate tunable influence.
                        self.log.info(
                            '  ℹ NOTE: More context switches observed '
                            '(%.1f%%) — may be workload-dominated; '
                            'check CPU migrations for tunable effect',
                            cs_change)
