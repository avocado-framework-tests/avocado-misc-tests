...
# Modified by: Samir M <samir@linux.ibm.com>
#
...
class Stressngcpu(Test):
    ...
    def setUp(self):
        ...
        # Determined once in setUp; avoids re-probing chrt for every PID.
        self._chrt_available = self._probe_chrt_availability()
        smm = SoftwareManager()
    ...
    def _probe_chrt_availability(self):
        """
        Check once whether the ``chrt`` utility is present on this system.

        Called from :meth:`setUp` so the result is cached in
        ``self._chrt_available`` and consulted by
        :meth:`_verify_policy_for_pids` to skip the secondary check early
        without spawning a subprocess for every PID of every transition.

        Returns ``True`` if chrt is found and executable, ``False`` otherwise.
        """
        try:
            ...
            self.log.warning(
                ...
                "secondary policy verification will be skipped for the "
                "entire test run")
            return False
        except subprocess.CalledProcessError:
            # chrt --version may exit non-zero on some versions; that still
    ...
    def _set_policy_for_pids(self, pids, policy):
        ...
        for pid in pids:
            if not self._pid_alive(pid):
                # Worker exited between transitions — this is expected for
                # short-lived stressors or under RT throttling.  Do NOT
                # record a set-policy failure; only genuine scheduler errors
                # should count.
                self.log.debug(
    ...
    def _verify_policy_for_pids(self, pids, expected_policy):
        ...
        *expected_policy*.
        ...
        for pid in pids:
            if not self._pid_alive(pid):
                # Worker exited before verification — expected when the
                # stress-ng timeout elapses mid-transition.  Not a failure.
                self.log.debug(
            ...
            # --- secondary: chrt tool (optional) ---
            # Skip entirely if chrt is not installed (checked once in setUp).
            if self._chrt_available:
                chrt_ok, chrt_actual = self._verify_policy_via_chrt(
                    pid, expected_policy)
                if not chrt_ok:
                    if chrt_actual in ("chrt-not-installed",
                                       "chrt-error", "parse-error"):
                        # Tool temporarily unavailable; primary result is
                        # authoritative — log at debug only, do not fail.
                        self.log.debug(
                            "PID %d: chrt secondary check skipped (%s)",
                            pid, chrt_actual)
                    else:
                        # Genuine policy mismatch reported by chrt.
                        failures.append(
                            "PID %d: chrt mismatch — expected %s got %s"
                            % (pid, policy_name, chrt_actual))
                else:
                    self.log.debug(
                        "PID %d: chrt verification OK — %s",
                        pid, chrt_actual)
    ...
    def test_runtime_policy_change(self):
        ...
        - ``chrt`` utility is optional; when absent the secondary check is
          skipped and only the primary kernel syscall result is used.
        ...
        if not child_pids:
            # Capture the PID *before* _terminate_stressng() nulls the ref.
