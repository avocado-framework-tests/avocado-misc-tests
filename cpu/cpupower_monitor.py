...
class CpupowerMonitor(Test):
    ...
    def setUp(self):
        self.output = self.run_cmd_out("cpupower idle-info --silent")
        for line in self.output.splitlines():
    ...
    def test_workload(self):
        ...
        for i in range(len(self.states_list)):
            zero_nonzero = self.check_zero_nonzero("cpupower monitor | grep -v '[A-Z]'", i + 1)
            if zero_nonzero:
        ...
        zero_nonzero = 0
        for i in range(len(self.states_list)):
            zero_nonzero = zero_nonzero + self.check_zero_nonzero("cpupower monitor | grep -v '[A-Z]'", i + 1)
        if zero_nonzero:
