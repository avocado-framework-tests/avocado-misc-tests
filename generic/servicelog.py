...
class servicelog(Test):
    ...
    is_fail = 0
    url = "http://ausgsa.ibm.com/projects/p/plx/FVT/rtas-events"
    ...
    def test_servicelog(self):
        ...
        rtas_events = ["v6_memory_info", "v6_power_error2", "v6_fw_predictive_error",
                       "v3_fan_failure", "v6_dump_notification", "v6_platform_error2",
                       "v6_eeh_info", "v6_cpu_guard", "v6_platform_info"]
        for event in rtas_events:
            rtas_file = self.fetch_asset("%s/%s" % (self.url, event), expire='10d')
