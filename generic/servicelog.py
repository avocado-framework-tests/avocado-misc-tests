...
#          Lucas Meneghel Rodrigues (lucasmr@br.ibm.com) (Current maintainer)
#          Hariharan T.S. <hari@linux.vnet.ibm.com> (ported to avocado)
#          Pavithra <pavrampu@linux.vnet.ibm.com>
...
class servicelog(Test):
    ...
    is_fail = 0
    EVENTS_PATH = os.path.dirname(__file__) + "/servicelog.py.data/rtas_events/"
    ...
    def test_servicelog(self):
        ...
        for event in rtas_events:
            ...
            self.run_cmd("servicelog --help")
            self.run_cmd("servicelog --version")
            self.run_cmd("servicelog --query='id=1'")
            self.run_cmd("servicelog --id=1")
            self.run_cmd("servicelog --query='severity>=$WARNING AND closed=0'")
            self.run_cmd("servicelog --serviceable=yes")
            self.run_cmd("servicelog --serviceable=no")
            self.run_cmd("servicelog --serviceable=all")
            self.run_cmd("servicelog --query='serviceable=1 AND closed=0'")
            self.run_cmd("servicelog --repair_action=yes")
            self.run_cmd("servicelog --repair_action=no")
            self.run_cmd("servicelog --repair_action=all")
            self.run_cmd("servicelog --repair_action=yes")
            self.run_cmd("servicelog --verbose --repair_action=no")
            self.run_cmd("servicelog --event_repaired=no")
            self.run_cmd("servicelog --severity=4")
            self.run_cmd("servicelog_manage --truncate notify --force")
        if self.is_fail >= 1:
    ...
    def test_servicelog_notify(self):
        ...
        if not os.path.exists(TMP_DIR):
            os.makedirs(TMP_DIR)
        NOTIFY_SCRIPT = TMP_DIR + "notify_script.sh"
