...
class RASTools(Test):
    ...
    @skipIf(IS_POWER_NV or IS_KVM_GUEST, "Skipping test in PowerNV platform")
    ...
    def test10_ppc64_cpu(self):
        ...
        if not process.system("ppc64_cpu --smt", ignore_status=True):
        ...
        if 'pSeries' not in open('/proc/cpuinfo', 'r').read():
