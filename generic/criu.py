...
class CRIU(Test):
    ...
    def setUp(self):
        sm = SoftwareManager()
        dist = distro.detect()
        packages = ['gcc', 'make', 'protobuf', 'protobuf-c', 'protobuf-c-devel', 'protobuf-compiler', 'protobuf-devel', 'protobuf-python', 'libnl3-devel', 'libcap-devel', 'libaio-devel']
        if dist.name != 'redhat':
    ...
    def test(self):
        ...
        process.run("./zdtm.py run -a --report sergeyb --keep-going", ignore_status=True, sudo=True)
        logfile = os.path.join(self.logdir, "stdout")
        failed_tests = process.system_output("grep -w FAIL %s" % logfile, shell=True, ignore_status=True)
        if failed_tests:
