...
class Package_check(Test):
    ...
    def test(self):
        if self.dist.name == 'rhel':
            ...
            for package in self.packages:
                if "anaconda" in process.system_output("yum list installed | "
                    ...
                    self.is_fail += 1
        elif self.dist.name == 'Ubuntu':
            self.packages.extend(['librtas2'])
            for package in self.packages:
        ...
        if self.is_fail >= 1:
            self.fail("%s package(s) not installed by default" % self.is_fail)
