...
class Stressng(Test):
    ...
    def return_status(self, return_code, test):
        if (return_code != 0):
            if (return_code == 137):
                self.fail("== >  %s timed out with error code" % (return_code, test))
            else:
                self.fail("== >  %s failed with error code" % (return_code, test))
    ...
    def vmstat_result(self, filename):
        ...
        Perf_data = process.system_output(cmd, ignore_status=True, sudo=True, shell=True)
    ...
    # Load the cpu using stressng and capture perf and vmstat data
        ...
        process.run("rm -rf /tmp/stressng_output* /tmp/data*", ignore_status=True, sudo=True)
        ...
        for load in self.cpu_per.split() :
            ...
            if self.is_process_running("stress-ng"):
                ...
                for iter in range(1,6):           
                    self.log.info("Running stress-ng CPU load %s for iteration %s" % (load, iter))
                ...
                self.log.info(" ***************************************************************************************")
                self.log.info(" Summary for CPU Load %s for %s iterations" % (load, iter))
                self.log.info(" Perf data %s   <=====> vmstat data %s" % ( perf_iterations, vmstat_iterations))
                self.log.info(" Perf Average %s <=====> vmstat Average %s " % (Perf_average, vmstat_average))
                self.log.info(" ***************************************************************************************")
                ...
                if change_percent > 2:
                    self.fail(" CPU load %s Failed with percentage %s difference" % (load, change_percent)) 

            else:
                self.log.info("stress-ng is ether not running or all the process are now killed ")

        self.log.info("=====================================================")

    def tearDown(self):
         process.run("rm -rf /tmp/stdout /tmp/stderr /tmp/data* /tmp/stressng_output*", ignore_status=True, sudo=True)
