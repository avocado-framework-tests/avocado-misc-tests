...
class test_eliminate_domain_suffix(Test):
    ...
    def setUp(self):
        ...
        if os.path.exists("%s" % self.event_sysfs):
            ...
            raise self.cancel("%s doesn't exist.This feature is supported\
        ...
        if "You may not have permission to collect\
            ...
            raise self.cancel("Please enable lpar to allow collecting\
    ...
    def test(self):
        ...
        if self.nfail != 0:
            raise self.error('Failed to verify domain name suffix\
                             in event names')
    ...
    def event_w_domain_param(self):
        ...
        if "Performance counter stats for 'CPU(s) 0'" not in result1.stderr:
            self.nfail += 1
    ...
    def check_all_domains(self):
        for domain in range(1, 6):
            ...
            if "Performance counter stats for 'CPU(s)\
                    0'" not in result1.stderr:
                self.nfail += 1
    ...
    def event_w_chip_param(self):
        event_out = process.run("cat\
            %s/events/PM_XLINK_CYCLES" % self.event_sysfs)
    ...
    def event_wo_chip_param(self):
        output_chip_missing = self.event_stat1('hv_24x7/PM_XLINK_CYCLES,\
            domain = 1/ /bin/true')
        if "Required parameter 'chip' not\
                specified" not in output_chip_missing.stdout:
            self.nfail += 1
            self.log.info('FAIL : perf unable to detect chip\
                          parameter missing')
        else:
            self.log.info('perf detected chip parameter missing')
        output_chip = self.event_stat1('hv_24x7/PM_XLINK_CYCLES,\
    ...
    def event_helper(self, event):
        search_suffix = process.run('ls %s/events |grep -E\
            %s' % (self.event_sysfs, event), ignore_status=True)
    ...
    def event_stat(self, cmd):
        result1 = process.run('%s%s'
                              % (self.perf_stat, cmd), ignore_status=True)
        return result1

    def event_stat1(self, cmd):
        result1 = process.run('%s%s'
                              % (self.perf_args, cmd), ignore_status=True)
        return result1
