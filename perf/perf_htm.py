...
class PerfHTMTrace(Test):
    ...
    def setUp(self):
        ...
        self.htm_workdir = os.path.join(self.outputdir, 'htm_work')
        os.makedirs(self.htm_workdir, exist_ok=True)
        ...
        self.log.info(
            "HTM Test Config: Node: %d, Chip: %d, Core: %d, Mode: %s, Duration: %ds, Workdir: %s",
            self.node, self.chip, self.core, self.trace_mode, self.trace_duration, self.htm_workdir
        )
            ...
            if result.exit_status == 0:
                ...
                if len(parts) >= 4:
                    ...
                    try:
                        ...
                        if available_gb < 10:
                            self.cancel(
                                "Low disk space: %dGB available (<10GB required)" % available_gb)
                    except ValueError:
                        pass
        except Exception:
            pass
        ...
        for attempt in range(1, retries + 1):
            ...
            try:
                ...
                if attempt < retries:
                    ...
                    return PhypResult(b"", exit_code=1)
    ...
    def tearDown(self):
        ...
        if hasattr(self, 'phyp_host') and self.phyp_host:
            has_topology = all(
                ...
                for attr in ('node', 'chip', 'core')
            )
            if has_topology:
                self.log.info(
                    "Deconfiguring HTM on phyp: Node: %d, Chip: %d, Core: %d",
                    self.node, self.chip, self.core
                )

                # Deconfigure HTM tracing according to trace_mode
                deconfigure_mode = 'llat' if hasattr(self, 'trace_mode') and self.trace_mode == 'llat' else 'inst'
                deconfigure_cmd = 'htm -deconfigure %s -n %d -p %d -c %d' % (
                    deconfigure_mode, self.node, self.chip, self.core
                )
