...
class Linuxtrace(Test):
    ...
    def run_cmd_out(self, cmd):
        logfile = "%s/ftracetest.log" % self.teststmpdir
    ...
    def test_ftrace(self):
        ...
        '''
        self.args = self.params.get('args', default='00basic')
        self.log.info("ftrace tests...: %s" % self.args)
        cmd = "./ftracetest -l %s ./test.d/%s" % (self.teststmpdir, self.args)
        self.run_cmd_out(cmd)
