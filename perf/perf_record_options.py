...
class PerfRecordOptions(Test):
    ...
    def get_src_options(self):
        ...
        # tests/shell
        cmd = f"grep -r 'perf record' {self.sourcedir}/tests || true"
    ...
    def run_and_check(self, opt):
        ...
        # Skip unsupported infra (cgroup / bpf / smi-cost / interval-clear)
    ...
    def test_perf_record_options(self):
        ...
        if not yaml_provided:
            self.log.info(
                "No YAML file provided, running plain perf record and exiting")
            result = process.run(
                "perf record -o /tmp/perf.data -- sleep 2",
                shell=True,
                ignore_status=False)
            if result.exit_status != 0:
                self.fail(f"Plain perf record failed: {result.stderr_text}")
            return
