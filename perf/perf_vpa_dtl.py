...
class PerfVpaDtl(Test):
    ...
    def setUp(self):
        ...
        if platform.machine() not in ['ppc64le', 'ppc64']:
            self.cancel("vpa_dtl test is supported only on PowerPC (ppc64/ppc64le)")
        ...
        elif 'debian' in detected_distro.name.lower():
        ...
        for package in deps:
            if not smm.check_installed(package) and not smm.install(package):
                if package == 'stress-ng':
                    self.log.warning("stress-ng is not available; preempt/fault workloads may not generate events")
                else:
        ...
        self.fault_duration = int(self.params.get('fault_duration',
                                                  default=90))
        self.preempt_cpu_factor = 4
        self.fault_cpu_factor = 2
        self.fault_vm_workers = 8
        self.fault_vm_bytes = '90%'
        self.record_retries = 3
        self.retry_sleep = 2
        self.inter_event_sleep = 2
    ...
    def _record_event(self, event, duration, workload_cmd=None):
        ...
        for attempt in range(1, self.record_retries + 1):
            with tempfile.NamedTemporaryFile(prefix='perf_vpa_dtl_', suffix='.data', delete=False) as f_data:
                perf_data = f_data.name
            with tempfile.NamedTemporaryFile(prefix='perf_vpa_dtl_', suffix='.report', delete=False) as f_dump:
                perf_dump = f_dump.name
