...
from avocado.utils import distro, process, pci , genio
...
class datatype_profiling(Test):
    ...
    def check_dependencies(self):
        ...
        detected_distro = distro.detect()
        if "rhel" in detected_distro.name.lower(): is_rhel = True;
        if "suse" in detected_distro.name.lower(): is_sles = True;
    ...
    def test_datatype_profiling(self):
        ...
        for cmd in record_cmds:
            ...
            self.check_perf_report_headers("perf report -i /tmp/perf_output.data -s symbol,type,typeoff")
            self.check_perf_annotate_headers("perf annotate -i /tmp/perf_output.data --data-type")
            self.check_perf_annotate_headers("perf annotate -i /tmp/perf_output.data --data-type --insn-stat", check_insn_stat=True)
