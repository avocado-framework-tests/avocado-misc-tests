...
class KDUMP(Test):
    ...
    def test(self):
        ...
        smt_cmd = "ppc64_cpu --smt=%s" % self.smt_value
        session_int.run(smt_cmd)
        session_int.run("ppc64_cpu --smt")
        ...
        crash_dir = self.run_cmd_out("cat %s | grep drwxr | awk '{print $NF}' | tail -1" % log_file)
        print("crash_dir = %s", crash_dir)
        ...
        session_int.session.kill()
        session_check.session.kill()
