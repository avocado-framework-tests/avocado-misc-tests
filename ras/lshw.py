    ...
    def test_accelerator(self):
        ...
        for line in lshw_accelerator_output.splitlines():
            ...
            if line.startswith("*-accelerator"):
                ...
                current_entry = {}
            elif current_entry is not None and line and ":" in line:
        ...
        if not entries:
            self.is_fail += 1
            self.fail_cmd.append("No accelerator entries parsed from lshw output")
            self.error_check()
        ...
        for i, entry in enumerate(entries):
            ...
            if 'description' in entry and 'processing accelerators' not in entry['description'].lower():
                self.is_fail += 1
                self.fail_cmd.append(f"Accelerator {i}: Invalid description")
            if 'product' in entry and 'Spyre Accelerator' not in entry['product']:
                self.is_fail += 1
